  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={handleSubmit} action="" className='w-full max-w-lg p-4 bg-white rounded-lg-border border-gray-200 '>
        ...
        <textarea rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='e.g., watch or spoon , only single object name' required value={object} onChange={(e) => setObject(e.target.value)} />

        <button disabled = {loading} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 rounded-md mt-6  cursor-pointer" type='submit'>
